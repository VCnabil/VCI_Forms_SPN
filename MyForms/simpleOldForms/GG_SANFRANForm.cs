...
namespace VCI_Forms_SPN
{
    //tkbr_Port_AhdRev
    //tkbr_Stbd_AhdRev
    public partial class GG_SANFRANForm : Form
    {
        ...
        public GG_SANFRANForm()
        {
            ...
            #endregion
            tkbr_Port_AhdRev.ValueChanged += tkbr_Port_AhdRev_ValueChanged;
            tkbr_Stbd_AhdRev.ValueChanged += tkbr_Stbd_AhdRev_ValueChanged;
        }
        private void tkbr_Port_AhdRev_ValueChanged(object sender, EventArgs e)
        {
            int value = tkbr_Port_AhdRev.Value;

            if (value >= 0)
            {
                spN_AHDp.Value= value;
                spN_REVp.Value = 0;
            }
            else
            {
                spN_AHDp.Value = 0;
                spN_REVp.Value = (Math.Abs(value));
            }
        }

        private void tkbr_Stbd_AhdRev_ValueChanged(object sender, EventArgs e)
        {
            int value = tkbr_Stbd_AhdRev.Value;

            if (value >= 0)
            {
                spN_AHDs.Value = (Math.Abs(value));
                spN_REVs.Value = 0;
            }
            else
            {
                spN_AHDs.Value = 0;
                spN_REVs.Value = (Math.Abs(value));
            }
        }
